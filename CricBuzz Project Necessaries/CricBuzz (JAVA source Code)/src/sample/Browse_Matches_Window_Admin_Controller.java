...
public class Browse_Matches_Window_Admin_Controller {
    ...
    public void Update_Score_Button_Clicked(ActionEvent actionEvent)
    {
        Browse_Match selected_Match = (Browse_Match) All_Matches_Tableview.getSelectionModel().getSelectedItem();
        ...
        if(selected_Match.getStatus().equalsIgnoreCase("UPCOMING")
                || selected_Match.getStatus().equalsIgnoreCase("RECENT") )
        {
            try {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("ERROR");
                alert.setHeaderText("Wrong Match is selected");
                alert.setContentText("Please Select a Live Match");
                Optional<ButtonType> result = alert.showAndWait();
        ...
        try {
            ...
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(getClass().getResource("Update_Score_DialogBox.fxml"));
            DialogPane root = loader.load();
            Update_Score_DialogBox_Controller controller = loader.getController();
            controller.init(Selected_Match_ID);
    ...
    public void Notify_Button_Clicked(ActionEvent actionEvent)
    {

        Browse_Match selected_Match = (Browse_Match) All_Matches_Tableview.getSelectionModel().getSelectedItem();

        if(selected_Match == null || !selected_Match.getStatus().equalsIgnoreCase("UPCOMING"))
        {
            try {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("ERROR");
                alert.setHeaderText("Selection Error");
                alert.setContentText("Please Select a Upcoming Match");
                Optional<ButtonType> result = alert.showAndWait();
        ...
        //A Upcoming Match is Selected


        try {
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("INFORMATION");
            alert.setHeaderText(selected_Match.getMatch_Title()+"    "+selected_Match.getFormat());
            alert.setContentText("Press OK to Turn on Notification For this Match");
            Optional<ButtonType> result = alert.showAndWait();

            if (result.get() == ButtonType.OK) {

                //getting the selected Match_id
                OracleConnect oc=null;
                int Selected_Match_ID =0;
                try {
                    oc = new OracleConnect(); //Connecting with oracle database
                    ...
                    String Admin_Match_id_update = "UPDATE ADMIN SET" +
                            " MATCH_ID = "+Selected_Match_ID+" WHERE ID = "+Main.Current_Admin_id;

                    oc.updateDB(Admin_Match_id_update);
